...
class MarketMaker:
    def __init__(self, book, half_spread=0.05, max_skew=0.20, scale=50,
                 fallback_price=100.0, order_qty=50):
        self.book = book
...
def step(self):
    ...
    self.last_mid = self.book.mid_price() if self.book.mid_price() is not None else self.last_mid
    ...
    self.history.append((self.last_mid, self.inventory, self.cash))
